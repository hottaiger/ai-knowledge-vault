...
> [!quote] 一句话理解
> cc-connect 是"中间层本地进程"——飞书 IM（用户入口）→ cc-connect（消息路由/会话/权限）→ Cursor Agent CLI（执行引擎 spawn 给 LLM）→ LLM 云端大脑。三层各管一摊，cc-connect 唯一关心的是怎么 spawn 子进程喂 prompt、收 JSON stream。

## 核心观点

- **三层架构**：cc-connect（中间层，本地进程）跟飞书 IM 保持 WebSocket 长连接；维护用户↔会话映射（`~/.cc-connect/sessions/`）；做权限检查（`allow_from` / `admin_from`）；把消息拼成完整 prompt，spawn 子进程喂给 Cursor Agent CLI；监听 JSON stream 输出实时回传飞书。Cursor Agent CLI 是执行层，给 LLM 提供工具列表、执行工具调用。LLM 在云端只"说要用什么工具"，不直接动文件。
- **一条消息的完整生命周期**：飞书 WebSocket → cc-connect 解析+权限+会话 → spawn `agent --print --output-format stream-json --resume <id> -p "..."` → LLM tool use loop（Read → 决定 → Write → 跑测试 → 修）→ JSON stream 输出 → cc-connect 实时更新飞书卡片。
- **session 两层**：cc-connect 的 session（`s1`、`s2`...对应不同飞书聊天）存在 `~/.cc-connect/sessions/`；Cursor Agent 的 `agent_session_id` 通过 `--resume` 接续上下文。用户发 `/new` → cc-connect 生成新 session + Cursor 开新对话。
- **work_dir**：Cursor Agent CLI 启动时 `cd` 到的目录，所有相对路径基于它；`.cursor/rules`、`.cursorrules` 从那读；cc-connect 的 `config.toml` 里 `[projects.agent.options] work_dir = "..."` 配置。
- **trust 机制**：Cursor Agent CLI 自己的安全机制，跟 cc-connect / Cursor IDE 都无关。首次访问未 trust 目录会弹交互式确认等待 stdin——无 stdin（如 cc-connect 子进程）就死锁。`--force` / `-f` / `--yolo` 跳过 trust + 自动放行所有工具；`--trust` 只跳过 trust 检查。cc-connect `mode = "force"` 自动给 spawn 的 agent 加 `--force`；`"default"` 不加，自己跑会卡。
- **Cursor IDE vs CLI**：共用同一个大脑、同一个 Agent 服务、同一套工具、同一个云端 LLM，区别只在入口、上下文丰富度、改动审核方式。CLI = 打工宿舍、IDE = 豪华办公室。cc-connect 不关心 Cursor Agent 住哪儿，只关心怎么 spawn 子进程把 prompt 喂进去、把 JSON 收回来。
- **必须用 `--print` + `stream-json` 的原因**：`--print` 非交互模式（CLI 默认 TUI 聊天界面等你打字）；`stream-json` 把每步输出成 JSON 行（一行一个事件），cc-connect 能实时解析、区分"思考/工具调用/最终回复"——用默认 `text` 输出体验差很多。
...
![三层架构](../inbox/manual/processed/cc-connect-architecture.png)
...
- cc-connect 避坑指南（实战 7 条）：`2026-06-22-cc-connect安装配置避坑指南`
