---
date: 2026-03-10
source: Claude Code course notes
source_type: video
source_url: https://claude.com/code
tags:
- Claude Code
- 工作流
- 第二大脑
...
Claude Code = 基于工具的 AI 编程助手，核心优势是工具使用能力。

**1. 工具使用系统**
- 语言模型本身只能处理文本，无法直接读写文件或执行命令
- 工具使用系统让 AI 能执行实际操作（读取文件、运行命令、浏览器控制等）
- Claude 在工具组合使用上优于其他模型

**2. 上下文管理**
- Claude.md 文件：项目摘要/架构/关键文件，每请求自动包含
- 三种级别：Project（团队共享）、Local（个人）、Machine（全局）
- # 内存模式：自然语言编辑 Claude.md
- @ 符号：精确引用特定文件而非让 AI 搜索

**3. 变更管理**
- 截图粘贴：Control-V 贴图让 AI 理解 UI
- Plan Mode：Shift+Tab 两次，详细实现计划
- Thinking Mode："Ultra think"触发，深度推理
...
**5. 自定义命令**
- 位置：.Claude/commands/ 文件夹
- 用 $arguments 接收参数
- 用于自动化重复任务

**6. MCP 服务器扩展**
- 添加外部工具集（如 Playwright 浏览器控制）
- claude mcp add [name] [start-command] 安装

**7. GitHub 集成**
- PR review 自动代码审查
- @Claude 在 issue/PR 中分配任务

**8. Hooks 机制**
- Pre-tool：工具执行前，可阻止
- Post-tool：工具执行后，提供反馈
- 典型用法：阻止读取 .env、类型检查、重复代码检测

**9. Claude Code SDK**
- CLI/TypeScript/Python 接口
- 默认只读，写权限需手动开启

## 我的思考

这篇示例条目展示了知识库推荐的写法：保留完整原始内容，再把可复用的核心观点抽到前面，方便 AI 与人类都快速定位重点。
