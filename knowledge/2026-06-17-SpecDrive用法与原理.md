...
> [!quote] 一句话理解
> SpecDrive 给 OpenSpec/Comet 加了两道关键门：`build_pause=plan-ready` 让"计划已就绪 → 构建未开始"成为显式的 **human-in-the-loop** 节点；`build_command` 让 guard 自动跑项目级构建命令，但**不会自动写**——必须由人或脚本显式配置。

## 核心观点

- **确认设计方案时只看 3 个文件**：(1) `openspec/changes/<change>/.specdrive/handoff/brainstorm-summary.md` 看最终结论；(2) `docs/superpowers/specs/<date>-<change>-design.md` 看完整设计；(3) `openspec/changes/<change>/specs/<spec>.md` 看需求约束做"规则级"复核。
- **`build_pause` 枚举值**：`null`（不暂停）/ `plan-ready`（计划已生成、构建未开始 → 暂停等待人工审核）。`plan-ready` 是**人工审批门控（human-in-the-loop）**的设计，把状态推进和技能编排解耦。
- **`build_command` 原理 = 读取 → 执行 → 回退**：按"change 的 `.specdrive.yaml` → 仓库根 `.specdrive.yaml` / `comet.yaml` → 自动探测"优先级读取；执行用 `bash -lc` 且做注入防护（禁 `; | & $ \`` ，允许字母数字 + 空格 + 引号 + 路径字符）。
- **自动探测只认 3 种栈**：`package.json` 有 build script → `npm run build`；`pom.xml` → `mvn compile`；`Cargo.toml` → `cargo build`。都不匹配 → FAIL。复杂命令（含 `|` 的 Jest `testPathPattern`）会被 guard 直接拒绝，需要放进包装脚本。
- **`verify_command` 与 `build_command` 分工**：verify 阶段先看 `verify_command`，没值再回退 `build_command`，再没值自动探测。典型配法：`build_command: pnpm build`（只编译）+ `verify_command: pnpm test`（验证跑测试）。
- **Skill 不要求 Agent 写 `build_command`**：所有 skill 只要求"跑"，不要求"写"。需要规范就在项目 README 或补充 skill 写清楚。
- **改字段用脚本**：change 级用 `specdrive-state set`，不要手改 `.specdrive.yaml`（仓库根配置除外）。

## 实践建议

1. **稳定项目**：在仓库根 `.specdrive.yaml` 写死 `build_command` / `verify_command`，避免 guard 猜错。
2. **复杂测试命令**：写 `scripts/verify.sh`，`build_command` 只指向脚本路径。
3. **不要指望 Agent 自动写**：skill 只要求"跑"，不要求"写"。
4. **改字段用脚本**：change 级用 `specdrive-state set`。
...
1. **`openspec/changes/<change>/.specdrive/handoff/brainstorm-summary.md`**
   先快速看最终结论，几分钟就能知道方案是不是对的。

2. **`docs/superpowers/specs/<date>-<change>-design.md`**
