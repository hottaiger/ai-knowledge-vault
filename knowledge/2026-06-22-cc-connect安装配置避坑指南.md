...
> [!quote] 一句话理解
> cc-connect 官方 INSTALL.md 漏了几个**只有在真实联调时才会暴露**的坑：飞书权限名带冒号还是带点、Cursor CLI OAuth 要单独走、`mode=default` 会卡 Workspace Trust、macOS 没有 `timeout` 命令、daemon 残留 lock 文件、飞书卡片回调不配按钮不响应、`allow_from` 白名单没日志。这 9 条都来自一次实战联调。

## 核心观点

- **飞书权限名照抄搜不到**：INSTALL.md 把**事件标识**（`im.message.receive_v1`，带点）和**权限标识**（带冒号）混了。搜权限要去 `docs/feishu.md` 第四章拿 5 个正确列表；事件订阅单独去「事件与回调」菜单加。
- **Cursor Agent CLI 看似可用实则未认证**：`agent --version` 不触发 API 调用，所以测不出来。CLI 的 OAuth 凭证（`~/.cursor/.cli-data/`）和 IDE 桌面凭证是分开的，IDE 登录不代表 CLI 登录过。`agent login` 必须用 `NO_OPEN_BROWSER=1 agent login &` 后台跑、浏览器授权。
- **⭐ Cursor `mode=default` 卡 Workspace Trust**：CLI 启动检查 work_dir trust 列表，未 trust 目录会卡 stdin 等待。cc-connect 子进程没 stdin 就死锁。**改成 `mode = "force"`** 跳过 trust + 自动放行所有工具——INSTALL.md 纯靠 debug 日志才能反推出来。
- **daemon install 后 Status: Stopped**：之前 kill 掉的进程没清 `~/.cc-connect/.config.toml.lock`，新 daemon 拿不到锁就退。`rm -f ~/.cc-connect/.config.toml.lock && cc-connect daemon start` 解决。
- **飞书 user open_id 从 daemon 日志拿**：给自己机器人发条消息，`cc-connect daemon logs | grep "message received"`，输出里 `user=ou_xxx` 就是你的 open_id（不是 `bot_open_id`）。最快的多人加白名单方式：让对方在飞书给机器人发 `/whoami`，机器人回他自己的 user ID。
- **macOS 没有 `timeout` 命令**（BSD userland），用 `cmd > log 2>&1 & PID=$!; sleep N; kill $PID` 绕开。macOS 在部分子命令里**不展开 `~`**，直接用绝对路径 `/Users/<user>/...`。
- **⭐ 飞书卡片回调没配 = 按钮点了没反应**：cc-connect 默认用交互卡片，按钮事件靠 `card.action.trigger` 回调。配完回调**必须重新创建应用版本并发布**，否则新配置不生效。临时绕开：在 `config.toml` 关闭 `enable_feishu_card = false` 回退纯文本模式。
- **团队成员用不了（`allow_from` 限制）**：`allow_from` 是**白名单**不是黑名单，被拦的消息**不写日志**，所以 `daemon logs` 看不到拒绝记录。配合 `mode = "force"` 时白名单内的人都能改代码——多人项目建议**每人一个 cc-connect 实例 + 不同 work_dir**，不要共用一个项目共享 `allow_from`。

## 实战 Checklist

按 INSTALL.md 顺序走的时候，额外确认：
