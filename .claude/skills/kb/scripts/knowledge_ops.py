...
CONCEPT_SPECS = [
    ...
    ConceptSpec(
        name="cc-connect",
        aliases=["cc-connect IM 桥", "Cursor Agent 飞书桥接"],
        keywords=["cc-connect", "Cursor Agent CLI", "飞书开放平台", "IM 集成", "WebSocket", "allow_from", "Workspace Trust"],
        tags=["cc-connect", "Cursor Agent CLI", "飞书"],
        definition="cc-connect 作为本地中间层进程，桥接飞书/钉钉等 IM 与 Cursor Agent CLI，让 IM 里直接驱动 AI 编码 Agent。",
        open_question="cc-connect 之外是否还需要一个通用 IM Agent 网关，能跨多平台同时挂多 Agent？",
        related_concepts=["OpenClaw与Agent编排", "Claude协作工作流"],
    ),
    ConceptSpec(
        name="Codex提示词工作流",
        aliases=["Codex Workflow", "Codex 自动化"],
        keywords=["Codex", "回顾提示词", "Chronicle", "Memories", "工作流识别"],
        tags=["Codex", "工作流/自动化"],
        definition="围绕 Codex 会话记录与 Chronicle，回顾重复工作流并封装成 Skill / Subagent / Automation 的提示词模式与方法论。",
        open_question="Codex Memories 之外，哪些跨会话模式值得结构化沉淀，哪些继续靠 Chronicle 即可？",
        related_concepts=["Claude Code Skills", "OpenSpec规范驱动"],
    ),
    ConceptSpec(
        name="OpenSpec规范驱动",
        aliases=["OpenSpec", "Comet", "SpecDrive"],
        keywords=["OpenSpec", "Comet", "SpecDrive", "spec drive", "openspec", "build_pause", "build_command", "delta spec", "主 spec"],
        tags=["OpenSpec", "Comet", "SpecDrive", "工作流/OpenSpec"],
        definition="OpenSpec / Comet / SpecDrive 一套：把提案、设计、任务、归档和验证写成可追溯的规范文件，配合 human-in-the-loop 门控推进 change。",
        open_question="OpenSpec 与传统 ADR / RFC 的边界在哪？什么时候用 OpenSpec，什么时候写 ADR 就够了？",
        related_concepts=["Claude Code Skills", "Claude协作工作流"],
    ),
    ConceptSpec(
        name="AI协作框架",
        aliases=["AI 协作边界", "AI 协作心智模型"],
        keywords=["AI 角球", "AI Coding", "Vibe Coding", "协作边界", "实习生", "ai-generated", "失忆的实习生"],
        tags=["AI/协作框架", "Vibe Coding"],
        definition="判断 AI 介入任务程度的框架（🟢🟡🟠🔴 四档），以及 AI 是'失忆的实习生'等协作心智模型与实践规则（AI 使用日志、单测补审）。",
        open_question="四档分级之外，是否需要更细粒度的'AI 介入深度'维度（比如 0-10 分）？",
        related_concepts=["Claude协作工作流", "Codex提示词工作流"],
    ),
    ConceptSpec(
        name="可视化工具链",
        aliases=["Visualization Stack", "图表生成"],
        keywords=["drawio", "draw.io", "mermaid", "excalidraw", "架构图", "可视化"],
        tags=["可视化", "架构图", "drawio"],
        definition="围绕架构图、流程图、时序图、思维导图等可视化的工具选型与自动生成 skill 集合。",
        open_question="哪些场景用文本 Mermaid 输出，哪些用 drawio 生成精细图？两者结合的工作流如何稳定下来？",
        related_concepts=["Claude Code Skills", "OpenSpec规范驱动"],
    ),
    ConceptSpec(
        name="MCP工具链",
        aliases=["Model Context Protocol"],
        keywords=["MCP", "Model Context Protocol", "MCP 工具", "MCP server", "tool call", "JSON-RPC"],
        tags=["MCP"],
        definition="Model Context Protocol 标准化 AI 调用外部工具/数据源。GitNexus、Playwright、Sequential Thinking、Figma MCP 等都基于 MCP 暴露能力。",
        open_question="MCP 工具数量增多后，Agent 的工具选择策略如何训练？安全白名单机制如何补强？",
        related_concepts=["OpenClaw与Agent编排", "代码知识图谱"],
    ),
    ConceptSpec(
        name="代码知识图谱",
        aliases=["Code Knowledge Graph", "GitNexus"],
        keywords=["GitNexus", "代码知识图谱", "knowledge graph", "graph", "符号", "调用图", "图查询"],
        tags=["GitNexus", "代码知识图谱", "知识图谱"],
        definition="把代码仓库解析为图（节点=符号，边=调用/导入/继承），让 AI 编码 Agent 通过查询替代 grep。代表工具：GitNexus。",
        open_question="代码知识图谱如何与运行时 trace、issue 图谱打通，形成完整的工程上下文网络？",
        related_concepts=["MCP工具链", "Claude协作工作流"],
    ),
]
